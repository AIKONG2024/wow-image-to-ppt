...
![결과](docs/examples/one-pun-scene.png)
...
우측 상단의 `한국어 | EN` 전환 버튼으로 UI 언어를 바꿀 수 있습니다.
...
1. Duct Tape(GPT-Image2), Nano Banana(나노바나나) 등으로 생성한 슬라이드 이미지를 업로드합니다.
2. `분석 실행`을 누릅니다.
...
2. Hugging Face에서 Access Token을 발급받습니다.
   - Hugging Face에 로그인합니다.
   - `Settings` → `Access Tokens` → `New token`을 누릅니다.
   - 권한은 `Read`로 만들고, 생성된 토큰을 복사합니다.
   - SAM3 모델 페이지가 접근 승인 방식이면 모델 페이지에서 access 요청도 먼저 완료합니다.
3. 아래 명령을 실행합니다.
